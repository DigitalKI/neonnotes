...
func start() -> void:
	# The editor can initialize plugins before its project/debugger session is
	# fully ready. Listening is editor-only and does not require Play, so retry
	# quietly instead of making startup look broken when the port is busy.
...
func _try_start_listening() -> void:
	...
	if err != OK:
		_retry_listen_at_ms = Time.get_ticks_msec() + 1000
		McpLog.log("[godot-mcp] bridge not available yet on 127.0.0.1:%d (err=%d); will retry" % [port, err])
		return
	_listening = true
...
func stop() -> void:
	...
	_server.stop()
	_listening = false
	_retry_listen_at_ms = 0


func _process(delta: float) -> void:
	if not _listening and Time.get_ticks_msec() >= _retry_listen_at_ms:
		_try_start_listening()
	if not _listening:
		return
	_poll_accum += delta
